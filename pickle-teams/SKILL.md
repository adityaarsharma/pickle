...
1. **Preconditions** — valid `TEAMS_TOKEN` (Graph) present; if auth fails, print the setup guide and **STOP** (never half-scan). Parse `TIME_RANGE` + `FOLLOWUP_MODE`. Load prefs (generic scoring if absent — never block). **Board memory:** load the saved To Do list destination (`todo_list_id`) from `state.json` and reuse it silently — if a valid id is already cached, never re-ask, re-discover, or re-create it; only find-or-create when the cache is missing or invalid.
